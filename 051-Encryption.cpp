...
For example, the sentence 's' = 'if man was meant to stay on the ground god would
have given us roots', after removing spaces is 54 characters long. SQRT(54) is between
...
and then displaying the next column and inserting a space, and so on. For example, the
...
std::string encryption(std::string s) {
    ...
    totalSubStrings = ceil(sqrt(s.length()));
    lengthSubStrings = floor(sqrt(s.length()));

    // Ensure that rows x columns >= L
    if((totalSubStrings * lengthSubStrings) < s.length())
        if(totalSubStrings < lengthSubStrings)
            totalSubStrings++;

    int start=0;
    for (int j=0; j<totalSubStrings; j++)
    {
        for(int i=start; i<s.length(); i+=totalSubStrings)
